...
class sale_order_add(osv.osv_memory):


    _name = 'sale.order.add'
    _columns = {
        'product_tmpl_id': fields.many2one('product.template', 'Modelo', required=True),
        'lines': fields.one2many('sale.order.add.line','tmp_id', 'Productos')        
    }
...
class sale_order_add_line(osv.osv_memory):
    ...
    _columns = {
        'tmp_id': fields.many2one('sale.order.add', 'Modelo', required=True) ,   
